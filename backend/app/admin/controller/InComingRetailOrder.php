...
/**
 * @ControllerAnnotation(title="商户进件-分销订单")
 */
class InComingRetailOrder extends AdminController
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            ...
            $count = $this->model
                ...
                ->count();
            $list = $this->model->withJoin(['user', 'orderInfo'])
                ->where($where)
                ->where('retail_num', '>', '0')
                ->where('pid', '<>', 0)
                ->where('pay_type', 2)
    ...
    {
        $id = input('id');
        $order = InComingOrder::where('id', $id)->find();
